  ...
  return (
    <main>
      ...
      <section className="work-section section" id="trabajos">
        ...
        <div className="work-grid">{works.map((work, i) => <Reveal as="article" key={work.image} delay={(i % 4) * 60} className={`${i === 0 ? "work-wide " : ""}${i === 6 ? "work-card-stable" : ""}`.trim()} onClick={() => setLightboxIndex(i)} role="button" tabIndex={0} onKeyDown={(e: KeyboardEvent) => { if (e.key === "Enter") setLightboxIndex(i); }}><img className="work-image" src={work.image} alt={work.title} loading={i < 3 ? "eager" : "lazy"} decoding="async" /><div className="work-overlay"><span>{work.tag}</span><h3>{work.title}</h3><i>0{i + 1}</i></div><div className="work-zoom" aria-hidden="true"><ZoomIn size={18} /></div></Reveal>)}</div>
