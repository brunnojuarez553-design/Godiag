...
const services = [
  ...
  { n: "05", title: "Osciloscopio y trazador de curvas", text: "Medición y análisis de señales para diagnosticar sensores, actuadores, cableado y módulos con evidencia técnica.", icon: Microscope, image: "https://res.cloudinary.com/dpiavcukm/image/upload/v1788992679/f2b7bbb3-4ad6-430f-8b5b-889b9bab9d3f_ijo3ma.jpg" },
  ...
  { n: "08", title: "Reparación de ABS", text: "Diagnóstico y reparación electrónica de módulos ABS y sus fallas asociadas.", icon: ShieldCheck, image: "https://res.cloudinary.com/dpiavcukm/image/upload/v1788305083/IMG_1041_i447vu.jpg" },
  { n: "09", title: "Reparación de BCM / TIPM", text: "Intervención de módulos de carrocería BCM y TIPM para fallas eléctricas, de comunicación y control.", icon: Microscope, image: "https://res.cloudinary.com/dpiavcukm/image/upload/v1788305082/IMG_1042_jaz2yl.jpg" },
  ...
  return (
    <main>
      ...
      <section className="work-section section" id="trabajos">
        ...
        <div className="work-grid">{works.map((work, i) => <Reveal as="article" key={work.image} delay={(i % 4) * 60} className={`${i === 0 ? "work-wide " : ""}${i === 6 ? "work-card-stable" : ""}`.trim()} onClick={() => setLightboxIndex(i)} role="button" tabIndex={0} onKeyDown={(e: KeyboardEvent) => { if (e.key === "Enter") setLightboxIndex(i); }}><Image src={work.image} alt={work.title} fill sizes="(max-width: 600px) 50vw, (max-width: 900px) 50vw, 25vw" style={{ objectFit: "cover" }} /><div className="work-overlay"><span>{work.tag}</span><h3>{work.title}</h3><i>0{i + 1}</i></div><div className="work-zoom" aria-hidden="true"><ZoomIn size={18} /></div></Reveal>)}</div>
